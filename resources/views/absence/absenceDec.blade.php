...
<div class="container mx-auto py-6">
    ...
    <div class="bg-white rounded-t-lg shadow">
        ...
        <div class="px-4 pb-3">
            <div class="inline-flex w-full rounded-lg bg-orange-500 p-1" role="group">
                <a href="?tab=acierie"
                    class="flex-1 px-4 py-2 rounded-lg font-bold text-center transition 
                        {{ $activeTab === 'acierie' ? 'bg-white text-orange-500 border border-orange-500' : 'bg-orange-500 text-white' }}">
                    ACIERIE
                </a>
                <a href="?tab=laminoire"
                    class="flex-1 px-4 py-2 rounded-lg font-bold text-center transition 
                        {{ $activeTab === 'laminoire' ? 'bg-white text-orange-500 border border-orange-500' : 'bg-orange-500 text-white' }}">
                    LAMINOIRE
                </a>
                <a href="?tab=administration"
                    class="flex-1 px-4 py-2 rounded-lg font-bold text-center transition 
                        {{ $activeTab === 'administration' ? 'bg-white text-orange-500 border border-orange-500' : 'bg-orange-500 text-white' }}">
                    ADMINISTRATION
                </a>
                <a href="?tab=chauffeur"
                    class="flex-1 px-4 py-2 rounded-lg font-bold text-center transition 
                        {{ $activeTab === 'chauffeur' ? 'bg-white text-orange-500 border border-orange-500' : 'bg-orange-500 text-white' }}">
                    CHAUFFEUR
                </a>
            </div>
        </div>
